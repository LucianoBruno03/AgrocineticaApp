...
import { fetchListShipper } from "@/api/request/search/Shipper";
import { Shipper, ShipperListResponse } from "@/types/search/Shipper";
...
export default function SearchScreen() {
  ...
  const getShipperQuery = useQuery<ShipperListResponse>({
    queryKey: ["getShipperQuery", searchedWord],
    queryFn: fetchListShipper,
  });

  const handleSelect = (item: Shipper) => {
  ...
  const renderContent = () => {
    ...
    if (
      searchedWord &&
      (!getShipperQuery.data?.data || getShipperQuery.data.data.length === 0)
    ) {
      return (
        <View style={styles.centeredContainer}>
          <ThemedText type="default" style={styles.statusText}>
    ...
    return (
      <FlatList
        data={getShipperQuery.data?.data}

...
export default function SearchScreen() {
  ...
  const getTransportEntityQuery = useQuery<TransportEntityListResponse>({
    queryKey: ["getTransportEntityQuery", searchedWord],
    queryFn: fetchListTransportEntity,
  });

  const handleSelect = (item: TransportEntity) => {
  ...
  const renderContent = () => {
    if (getTransportEntityQuery.isLoading) {
      return <LoaderWithText />;
    }

    if (getTransportEntityQuery.isError) {
      return (
    ...
    if (
      searchedWord &&
      (!getTransportEntityQuery.data?.data ||
        getTransportEntityQuery.data.data.length === 0)
    ) {
      return (
        <View style={styles.centeredContainer}>
    ...
    return (
      <FlatList
        data={getTransportEntityQuery.data?.data}

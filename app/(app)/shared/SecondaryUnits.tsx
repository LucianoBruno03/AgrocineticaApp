import { fetchListUnits } from "@/api/request/search/Units";
import { ThemedText } from "@/components/ThemedText";
import LoaderWithText from "@/components/ui/loaders/LoaderWithText";
import { useColorScheme } from "@/hooks/useColorScheme";
import useDebounce from "@/hooks/useDebounce";
import { Units, UnitsListResponse } from "@/types/search/Units";
...
export default function SearchScreen() {
  ...
  const getMainUnitsQuery = useQuery<UnitsListResponse>({
    queryKey: ["getMainUnitsQuery", searchedWord, entityId, "REMOLQUE"],
    queryFn: fetchListUnits,
    enabled: !!entityId,
  });

  const handleSelect = (item: Units) => {
    ...
    const newFormValues = {
      ...
      trailerId: item.id,
      trailerName: item.domain,
      scalableId: item.scalableId,
      scalableName: item.scalableName,
    };
  ...
  const renderContent = () => {
    if (getMainUnitsQuery.isLoading) {
      return <LoaderWithText />;
    }

    if (getMainUnitsQuery.isError) {
      return (
    ...
    if (
      searchedWord &&
      (!getMainUnitsQuery.data?.data ||
        getMainUnitsQuery.data.data.length === 0)
    ) {
      return (
        <View style={styles.centeredContainer}>
    ...
    return (
      <FlatList
        data={getMainUnitsQuery.data?.data}

...
export default function LocalitiesDropdown({ form }: { form: any }) {
  ...
  const getLocalitiesQuery = useQuery({
    queryKey: ["getLocalitiesQuery", false, form.getValues().provinceId],
    queryFn: fetchListLocalities,
    enabled: !!form.getValues().provinceId,
  });

  useEffect(() => {
    if (getLocalitiesQuery.data) {
      const rawData = getLocalitiesQuery.data?.data;
      if (Array.isArray(rawData)) {
        const dataDropdownMap = rawData.map((item: LocalitiesDropdownData) => ({
          label: item.name,
          value: item.id,
        }));
        ...
        const initialValue = dataDropdownMap.find(
          (item) => item.value === form.getValues().localityId
        );
        if (initialValue) {
  ...
  }, [getLocalitiesQuery.data, form.getValues().provinceId]);
    ...
    <View style={styles.container}>
      <View style={styles.inputWrapper}>
        ...
        <Dropdown
          ...
          labelField="label"
          valueField="value"
          placeholder={isFocus ? "Seleccione una localidad" : ""}

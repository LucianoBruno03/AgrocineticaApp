...
export interface BusinessDetailStatusDropdown {
  id: string;
  name: string;
  ord: number;
  active: boolean;
...
export default function BusinessDetailStatusDropdown({ form }: { form: any }) {
  ...
  useEffect(() => {
    if (getCountriesQuery.data) {
      ...
      if (Array.isArray(rawData)) {
        const dataDropdownMap = rawData.map(
          (item: BusinessDetailStatusDropdown) => ({
            label: item.name,
            value: item.id,
          })
        );

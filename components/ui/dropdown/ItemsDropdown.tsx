...
export interface ItemsDropdownData {
  id: string;
  name: string;
  ord: number;
  active: boolean;
...
export default function AnimatedItemsDropdown({ form }: { form: any }) {
  ...
  React.useEffect(() => {
    if (getItemsQuery.data) {
      const rawData = getItemsQuery.data?.data;
      if (Array.isArray(rawData)) {
        const dataDropdownMap = rawData.map((item: ItemsDropdownData) => ({
          label: item.name,
          value: item.id,
        }));
  ...
  }, [getItemsQuery.data, form.getValues().itemId]);

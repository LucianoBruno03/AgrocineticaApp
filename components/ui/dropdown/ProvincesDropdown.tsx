...
export default function ProvincesDropdown({ form }: { form: any }) {
  ...
  const getProvincesQuery = useQuery({
    queryKey: ["getProvincesQuery", false, form.getValues().countryId],
    queryFn: fetchListProvinces,
    enabled: !!form.getValues().countryId,
  });

  useEffect(() => {
    if (getProvincesQuery.data) {
      const rawData = getProvincesQuery.data?.data;
      if (Array.isArray(rawData)) {
        const dataDropdownMap = rawData.map((item: ProvincesDropdownData) => ({
          label: item.name,
          value: item.id,
        }));
        ...
        const initialValue = dataDropdownMap.find(
          (item) => item.value === form.getValues().provinceId
        );
        if (initialValue) {
  ...
  }, [getProvincesQuery.data, form.getValues().countryId]);
    ...
    <View style={styles.container}>
      <View style={styles.inputWrapper}>
        ...
        <Dropdown
          ...
          labelField="label"
          valueField="value"
          placeholder={isFocus ? "Seleccione una provincia" : ""}
          ...
          onChange={(item) => {
            ...
            setIsFocus(false);
            form.setValue("provinceId", item.value);
          }}
          renderLeftIcon={() => null}
        />
      </View>

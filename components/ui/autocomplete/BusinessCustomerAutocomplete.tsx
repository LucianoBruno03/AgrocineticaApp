...
interface BusinessCustomerAutoCompleteProps {
  form: any;
  error?: FieldError;
}

export default function BusinessCustomerAutoComplete({
  form,
  error,
}: BusinessCustomerAutoCompleteProps) {
  ...
  const [selectedValue, setSelectedValue] = useState<string | null>(
    form.getValues().businessUserName
  );

  useEffect(() => {
    const newValue = form.getValues().businessUserName;
  ...
  const handleSearch = () => {
    router.push({
      pathname: "/business/BusinessCustomer",
      params: {
